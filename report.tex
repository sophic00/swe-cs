...
The script \texttt{extract\_complexity.py} invokes \texttt{lizard}~\cite{yin2023} on each cloned repository, excluding test directories (\texttt{*/tests/*}, \texttt{*/test\_*}). For every Python file it records NLOC, token count, average CCN across all functions, and function count.

\paragraph{History mining.}
The script \texttt{extract\_history.py} uses \texttt{PyDriller}~\cite{spadini2018} to traverse every commit from the past two years.
