...
\bottomrule
\endlastfoot
\texttt{scripts/extract\_complexity.py} & Runs \texttt{lizard} over each repo; outputs static-analysis CSV \\
\texttt{scripts/extract\_history.py}    & Uses \texttt{PyDriller} to mine 2-year Git history; outputs churn CSV \\
\texttt{scripts/generate\_plots.py}     & Merges datasets, computes hotspot scores, produces 12 analytical plots \\
\texttt{scripts/run\_pipeline.py}       & End-to-end runner: clones repos, regenerates CSVs, and renders plots \\
\texttt{data/*\_complexity.csv}         & Raw static-analysis results (per repo) \\
\texttt{data/*\_history.csv}            & Raw history-mining results (per repo) \\
