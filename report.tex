...
\begin{tikzpicture}[remember picture, overlay]
  \fill[covernavy]
    (current page.north west) rectangle ([yshift=-0.7cm]current page.north east);
  \fill[covergold]
    ([yshift=-0.7cm]current page.north west) rectangle ([yshift=-0.85cm]current page.north east);
  \fill[coverblue]
    (current page.south west) rectangle ([yshift=0.8cm]current page.south east);
  \node[anchor=south, text=white, font=\small\bfseries]
...
\vspace{0.3cm}
{\color{covergold}\rule{0.72\textwidth}{1.4pt}\par}
...
\raggedright
\small
{\bfseries\color{covernavy}Course\par}
Software Engineering\par
\vspace{0.2cm}
{\bfseries\color{covernavy}Submitted By\par}
Vaibhav Sijaria\par
\vspace{0.2cm}
{\bfseries\color{covernavy}Registration No.\par}
23BCI0063\par
\end{minipage}%
\hfill
...
\raggedright
\small
{\bfseries\color{covernavy}Submission Date\par}
6 April 2026\par
\vspace{0.2cm}
{\bfseries\color{covernavy}Focus\par}
Hotspot analysis using complexity,\newline churn, and bug-fix history\par
\vspace{0.2cm}
{\bfseries\color{covernavy}Repository\par}
\url{https://github.com/sophic00/swe-cs}\par
\end{minipage}%
\vspace{0.25cm}
\end{minipage}%
}%
...
Cunningham~\cite{cunningham1992} introduced the \emph{technical debt} metaphor in 1992 to describe how expedient but imperfect code decisions create an implicit cost that must be repaid through future refactoring.
Kruchten et al.~\cite{kruchten2012} later formalised a taxonomy that distinguishes \emph{code debt}, \emph{design debt}, \emph{testing debt}, and \emph{documentation debt}.
Fowler~\cite{fowler2009} further distinguished between \emph{deliberate} and \emph{inadvertent} debt and between \emph{reckless} and \emph{prudent} debt, noting that even prudent decisions can accumulate debt over time.
...
McCabe~\cite{mccabe1976} proposed cyclomatic complexity (CCN) as a count of linearly independent paths through a function's control-flow graph.
Higher CCN implies more branches and, consequently, more potential for faults.
Empirical work by Basili and Perricone~\cite{basili1984} confirmed that modules with CCN~$>10$ are significantly more defect-prone, a threshold that remains widely cited.
...
Nagappan and Ball~\cite{nagappan2005} demonstrated that \emph{relative code churn} (the frequency and volume of changes to a file) is among the strongest predictors of post-release defects in large systems.
Their study of Windows Server 2003 found that churn-based models outperformed size-based models in predictive accuracy.
Hassan~\cite{hassan2009} extended this line of work by showing that the \emph{entropy of changes} (how spread out the modifications are) further improves fault-proneness prediction.

\subsection{Hotspot Analysis}

Tornhill~\cite{tornhill2015} synthesised complexity and churn research into a practical technique he calls \emph{hotspot analysis}: files that are \emph{both} complex and frequently changed are the ones most likely to harbour latent defects.
...
Spadini et al.~\cite{spadini2018} developed \texttt{PyDriller}, a Python framework for mining Git repositories, which we use in this study.
A common heuristic in MSR (Mining Software Repositories) research is to classify a commit as a \emph{bug-fix} when its message contains keywords such as ``fix'', ``bug'', ``issue'', ``resolve'', ``patch'', or ``error''~\cite{mockus2000}.
While this keyword-based approach is imperfect (it may miss some fixes and mis-classify some non-fixes), Mockus and Votta~\cite{mockus2000} showed that it correctly identifies roughly 60--70\% of true bug-fix commits, making it a practical proxy for defect density.

\subsection{Code Smells}

Fowler and Beck~\cite{fowlerbeck1999} catalogued recurring structural problems (\emph{code smells}) that indicate deeper design issues.
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Complexity dataset} (\texttt{data/*\_complexity.csv}): one row per source file, containing NLOC, token count, average cyclomatic complexity, and function count.
        Django: 857 files; Transformers: 2\,757 files.
  \item \textbf{History dataset} (\texttt{data/*\_history.csv}): one row per file that was modified at least once in the past two years, containing churn (commit count), bug-fix commit count, lines added, and lines deleted.
...
\bottomrule
\endlastfoot
\path{scripts/extract_complexity.py} & Runs \texttt{lizard} over each repo; outputs static-analysis CSV \\
\path{scripts/extract_history.py}    & Uses \texttt{PyDriller} to mine 2-year Git history; outputs churn CSV \\
\path{scripts/generate_plots.py}     & Merges datasets, computes hotspot scores, produces 12 analytical plots \\
\path{scripts/run_pipeline.py}       & End-to-end runner: clones repos, regenerates CSVs, and renders plots \\
\path{data/*_complexity.csv}         & Raw static-analysis results (per repo) \\
\path{data/*_history.csv}            & Raw history-mining results (per repo) \\
\path{plots/fig01,...,fig12}         & 12 PNG figures used in this report \\
\path{report.tex}                    & This document (LaTeX source) \\
\end{longtable}
...
The script \texttt{scripts/extract\_complexity.py} invokes \texttt{lizard}~\cite{yin2023} on each cloned repository, excluding test directories (\texttt{*/tests/*}, \texttt{*/test\_*}). For every Python file it records NLOC, token count, average CCN across all functions, and function count.

\paragraph{History mining.}
The script \texttt{scripts/extract\_history.py} uses \texttt{PyDriller}~\cite{spadini2018} to traverse every commit from the past two years.
...
\[
  ...
  \right)
\]
Equal weighting was chosen for simplicity and transparency; in a production setting, weights could be learned from labelled defect data~\cite{nagappan2005}.
...
\midrule
NLOC             & \texttt{lizard}     & Non-comment lines of code; proxy for module size. Large modules indicate potential God Class smell~\cite{fowlerbeck1999}. \\
Token count      & \texttt{lizard}     & Total lexical tokens; a finer-grained size measure than NLOC. \\
Avg.\ CCN        & \texttt{lizard}     & Mean cyclomatic complexity over all functions in the file. CCN\,$>$\,10 = moderate risk, CCN\,$>$\,20 = high risk~\cite{mccabe1976}. \\
Function count   & \texttt{lizard}     & Number of callable units; combined with NLOC helps detect God Modules. \\
Churn            & \texttt{PyDriller}  & Number of commits that touched the file in the past two years. Strong defect predictor~\cite{nagappan2005}. \\
Bug-fix count    & \texttt{PyDriller}  & Subset of churn commits whose message matches bug keywords~\cite{mockus2000}. \\
...
Complexity alone shows a weak or non-significant relationship with bug-fixes, which \emph{validates the hotspot model}: it is the combination of complexity and churn that matters, not either metric in isolation~\cite{tornhill2015}.
...
This confirms that, holding churn constant, complex code requires proportionally more corrective maintenance, supporting McCabe's original thesis~\cite{mccabe1976} and Basili and Perricone's empirical findings~\cite{basili1984}.
...
Figure~\ref{fig:churn_dist} reveals a heavy-tailed (near power-law) distribution of churn: a small fraction of files absorbs the vast majority of changes, consistent with the Pareto principle widely observed in software evolution research~\cite{hassan2009}.
...
Large files with many functions and elevated complexity are symptomatic of the \emph{God Module} code smell~\cite{fowlerbeck1999}.
...
Mockus and Votta~\cite{mockus2000} estimate its precision at roughly 60--70\%.
...
In practice, one metric may be a stronger predictor than the others for a given project; learning optimal weights from labelled defect data~\cite{nagappan2005} could improve ranking accuracy.
...
The hotspot technique is language-agnostic in principle, but the specific metric thresholds (e.g.\ McCabe's CCN=10 boundary) were calibrated on C/Fortran code~\cite{mccabe1976} and may not transfer directly to Python, where language idioms like list comprehensions can compress control flow.
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{A small number of files will dominate the risk landscape.}
        Nagappan and Ball~\cite{nagappan2005} and Tornhill~\cite{tornhill2015} predict that defects follow a Pareto distribution.
  ...
  \item \textbf{Churn will be a stronger predictor of bugs than complexity.}
        Prior work~\cite{nagappan2005,hassan2009} consistently finds churn superior to static metrics.
        \emph{Result: confirmed.}
  ...
  \item \textbf{The hotspot model will surface architecturally central modules.}
        Kazman et al.~\cite{kazman2015} showed that technical debt concentrates in architectural ``hubs.''
        \emph{Result: confirmed.}
...
W.\ Cunningham, ``The WyCash Portfolio Management System,'' \emph{ACM SIGPLAN OOPS Messenger}, vol.\ 4, no.\ 2, pp.\ 29--30, Dec.\ 1992.
doi:\ \url{https://doi.org/10.1145/157710.157715}

\bibitem{mccabe1976}
T.\ J.\ McCabe, ``A Complexity Measure,'' \emph{IEEE Trans.\ Software Engineering}, vol.\ SE-2, no.\ 4, pp.\ 308--320, Dec.\ 1976.
doi:\ \url{https://doi.org/10.1109/TSE.1976.233837}

\bibitem{basili1984}
V.\ R.\ Basili and B.\ T.\ Perricone, ``Software Errors and Complexity: An Empirical Investigation,'' \emph{Comm.\ ACM}, vol.\ 27, no.\ 1, pp.\ 42--52, Jan.\ 1984.
doi:\ \url{https://doi.org/10.1145/69605.2085}

\bibitem{nagappan2005}
N.\ Nagappan and T.\ Ball, ``Use of Relative Code Churn Measures to Predict System Defect Density,'' in \emph{Proc.\ 27th International Conference on Software Engineering (ICSE)}, pp.\ 284--292, 2005.
doi:\ \url{https://doi.org/10.1145/1062455.1062514}

\bibitem{hassan2009}
A.\ E.\ Hassan, ``Predicting Faults Using the Complexity of Code Changes,'' in \emph{Proc.\ 31st International Conference on Software Engineering (ICSE)}, pp.\ 78--88, 2009.
doi:\ \url{https://doi.org/10.1109/ICSE.2009.5070510}

\bibitem{tornhill2015}
A.\ Tornhill, \emph{Your Code as a Crime Scene: Use Forensic Techniques to Arrest Defects, Bottlenecks, and Bad Design in Your Programs}. Pragmatic Bookshelf, Mar.\ 2015.
ISBN:\ 9781680500387. \url{https://pragprog.com/titles/atcrime/your-code-as-a-crime-scene/}

\bibitem{kruchten2012}
P.\ Kruchten, R.\ L.\ Nord, and I.\ Ozkaya, ``Technical Debt: From Metaphor to Theory and Practice,'' \emph{IEEE Software}, vol.\ 29, no.\ 6, pp.\ 18--21, Nov.\ 2012.
doi:\ \url{https://doi.org/10.1109/MS.2012.167}

\bibitem{fowler2009}
M.\ Fowler, ``Technical Debt Quadrant,'' \emph{martinfowler.com}, Oct.\ 14, 2009; reposted Nov.\ 19, 2014.
\url{https://martinfowler.com/bliki/TechnicalDebtQuadrant.html}

\bibitem{fowlerbeck1999}
M.\ Fowler and K.\ Beck, \emph{Refactoring: Improving the Design of Existing Code}. Addison-Wesley Professional, Jun.\ 1999.
ISBN:\ 9780201485677. \url{https://www.informit.com/store/refactoring-improving-the-design-of-existing-code-9780201485677}

\bibitem{mockus2000}
A.\ Mockus and L.\ G.\ Votta, ``Identifying Reasons for Software Changes Using Historic Databases,'' in \emph{Proc.\ International Conference on Software Maintenance (ICSM)}, pp.\ 120--130, 2000.
doi:\ \url{https://doi.org/10.1109/ICSM.2000.883028}

\bibitem{spadini2018}
G.\ Spadini, M.\ Aniche, and A.\ Bacchelli, ``PyDriller: Python Framework for Mining Software Repositories,'' in \emph{Proc.\ 2018 26th ACM Joint Meeting on European Software Engineering Conference and Symposium on the Foundations of Software Engineering (ESEC/FSE)}, pp.\ 908--911, 2018.
doi:\ \url{https://doi.org/10.1145/3236024.3264598}

\bibitem{kazman2015}
R.\ Kazman, Y.\ Cai, R.\ Mo, Q.\ Feng, L.\ Xiao, S.\ Haziyev, V.\ Fedak, and A.\ Shapochka, ``A Case Study in Locating the Architectural Roots of Technical Debt,'' in \emph{Proc.\ 2015 IEEE/ACM 37th IEEE International Conference on Software Engineering (ICSE), vol.\ 2}, pp.\ 179--188, 2015.
doi:\ \url{https://doi.org/10.1109/ICSE.2015.146}

\bibitem{yin2023}
T.\ Yin, ``Lizard: A Code Complexity Analyzer,'' GitHub repository.
\url{https://github.com/terryyin/lizard} (accessed Apr.\ 2, 2026).
\end{thebibliography}
